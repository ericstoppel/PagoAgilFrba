...
IF OBJECT_ID('PUNTO_ZIP.PR_Get_Rubros') IS NOT NULL
DROP PROCEDURE PUNTO_ZIP.PR_Get_Rubros
GO

IF OBJECT_ID('PUNTO_ZIP.PR_Get_Sucursales') IS NOT NULL
DROP PROCEDURE PUNTO_ZIP.[PR_Get_Sucursales]
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [PUNTO_ZIP].[PR_Get_Rubros]
AS
  BEGIN TRY
	SELECT DISTINCT nombre FROM [PUNTO_ZIP].RUBRO
  END TRY
  BEGIN CATCH
    SELECT 'ERROR', ERROR_MESSAGE()
  END CATCH
...
AS
  BEGIN TRY
	 ...
	 SET nombre=@nombre, direccion=@direccion, codigo_postal=@codigo_postal, activo=@activo
	 WHERE id = @id
  END TRY
  BEGIN CATCH
    SELECT 'ERROR', ERROR_MESSAGE()
